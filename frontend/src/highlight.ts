...
// over a whole hit array instead of one element.

export function clearLinkImmediate(state: OverlayState, linkGroup: SVGGElement): void {
...
export function clearLink(state: OverlayState, linkGroup: SVGGElement, fade = false): void {
    if (!fade || !linkGroup.firstChild || prefersReducedMotion()) {
        clearLinkImmediate(state, linkGroup)
        return
    }
    state.linkKey_ = null
...
export function drawLink(state: OverlayState, linkGroup: SVGGElement, key: string, hits: Hit[]): void {
    if (key === state.linkKey_ && linkGroup.firstChild && !(linkGroup.firstElementChild as Element).classList.contains("masque-leave")) return
    clearLinkImmediate(state, linkGroup)
    for (const h of hits) {
