...
| **Release** | CHANGELOG freeze at `0.1.0` → Jonathan comments `@JuliaRegistrator register` on a **CI-green `main` commit** after the prep PR merges (CI is the sole author of `assets/overlay.js`) → TagBot creates the `v0.1.0` git tag. Do not register from the prep PR. | Julia ships the registered git tree. Tagging a pre-bundle or workflow-adding commit can publish a stale overlay or fail TagBot. Human steps in `docs/releasing.md`. |
