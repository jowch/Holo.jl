...
- `RegionInteractable` / `FunctionInteractable` — custom interactions, no JavaScript required; see [Custom interactions](@ref)
...
`:cairo`, live on `:webgl` (see [Backends](@ref)). Continuous pixel→data
...
[`examples/demo.jl`](https://github.com/jowch/Holo.jl/blob/main/examples/demo.jl) is a runnable gallery of every kind below plus the
...
[`examples/view_manip.jl`](https://github.com/jowch/Holo.jl/blob/main/examples/view_manip.jl) demonstrates sliders and gestures.
