...
**~~Never~~ Not invertible client-side (tier 3–4):** MeshScatter, Arrows3D, Surface, RainClouds, PolarAxis (continuous), Axis3D, decorations. PolarAxis discrete hits are projectable server-side (`transform_func`); continuous θ/r readout still needs the polar transform in JS — see `roadmap.md` M3. **Reconciled (2026-07-02, parity reframe):** the invertibility fact stands, but "Never" as scheduling is superseded by server-side (build-time) projection — Axis3D plus MeshScatter/Arrows3D are committed `roadmap.md` M3 scope; `Surface` stays deferred. PolarAxis discrete overlay parity has since shipped; continuous readout remains the follow-up.
