...
> - **The `@bind` view-commit baseline** below is the same kind of one-off: its driver lives in
>   the gitignored `spike/` directory, not in `bench/`, so it is a dated snapshot too, not
>   reproducible from anything committed to this repo.
...
headless Chromium (Playwright) — a one-off from the gitignored `spike/` driver, not
`bench/payload_envelope.jl`; see the header note above. Pluto's version matters here because the
double remount below is a Pluto-version-dependent bug (#83), not a Masque one.
...
release — issue #83's double remount, reproduced live on this Pluto version rather than only
inferred from its description. Adding the light-scene decode+paint floor measured elsewhere in
this file (~31–35 ms, see the round-trip table above) to the final remount gives a **derived**
(not separately measured) release-to-visible estimate of **≈230 ms p50** for today's `@bind`
view-commit path on this scene.
