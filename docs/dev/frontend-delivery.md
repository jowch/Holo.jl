...
| **Release** | When the API has stopped moving, Jonathan comments `@JuliaRegistrator register` on a **CI-green `main` commit** (CI is the sole author of both `assets/overlay.js` and `assets/masque-webgl.js`) → TagBot creates the `v0.1.0` git tag. Register a commit, never a branch head. | Julia ships the registered git tree. Tagging a pre-bundle or workflow-adding commit can publish a stale bundle or fail TagBot. |
