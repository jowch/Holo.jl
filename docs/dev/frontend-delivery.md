...
| **JS testing** | **vitest** for hit-test geometry **and** overlay visual recipes (wash / ring / halo / remount fade / `prefers-color-scheme` CSS); the **Pluto + Playwright** harness for browser integration in CI (`bind_click.mjs` / `click.mjs`). The local kind-sweep + polish-verify playbook (`docs/live-interaction-checklist.md`) covers interaction **and** visual on every backend — those drivers are not CI. | Geometry and chrome tokens are unit-testable in Node; live hover/click/`@bind` and on-screen recipes need a real Pluto + browser. |
...
| **CI** | One GitHub Action job is the **sole author** of both committed bundles (`assets/overlay.js` IIFE + `assets/holo-webgl.js` ESM, both built from `frontend/`): builds + commits on push to `main` (loop-safe via `GITHUB_TOKEN`); PRs (incl. forks, which can't push) fail if either asset is stale (`git diff --exit-code` on both paths). | Main head bundles are never stale; committing a local build becomes optional. |
| **Release** | CHANGELOG freeze at `0.1.0` → Jonathan comments `@JuliaRegistrator register` on a **CI-green `main` commit** after the prep PR merges (CI is the sole author of both `assets/overlay.js` and `assets/holo-webgl.js`) → TagBot creates the `v0.1.0` git tag. Do not register from the prep PR. | Julia ships the registered git tree. Tagging a pre-bundle or workflow-adding commit can publish a stale bundle or fail TagBot. Human steps in `docs/releasing.md`. |
