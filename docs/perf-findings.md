...
- **Multi-select / box-select** *(delivered, M4 commit `d05318f`)* — the M4 wire change is a
  ...
  2026-06-29 confirms all section-A manifest sizes are unchanged — see §A. K selected events ×
  ~tens of bytes on the inbound path; never a size concern. The contract change, not the size,
  was the work.
