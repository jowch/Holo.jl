...
[`docs/backend-comparison.md`](backend-comparison.md) (generated by `bench/vs_cairo.jl`). It quantifies
the Non-goals stance below: `:cairo` wins static small–mid 2D (smaller payload, no bundle, instant
first paint); `:webgl` wins large/animated/live 2D (flat ~30 ms server cost vs Cairo's up-to-2.2 s,
crossover ≈1 render at scatter-10k) and **owns** 3D + client-side pan/zoom/rotate (impossible on a
static PNG). They are **co-equal entry points**, not light-vs-heavy.
