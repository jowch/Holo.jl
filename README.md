...
> (`ViewInteractable`). Validated end-to-end in real Pluto on every supported backend —
> CairoMakie via [`examples/demo.jl`](examples/demo.jl), WGLMakie via
...
feature fork — and never both in the same session. The first-registration steps (Registrator
comment on a CI-green `main` commit, TagBot tag) are in [`docs/releasing.md`](docs/releasing.md).
